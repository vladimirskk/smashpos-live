...
(() => {
  ...
  const stylesheet =
    ...
    // touch-action: pan-x pan-y (not none) so a finger landing on the image can
    // still scroll the page vertically AND swipe a horizontal carousel. Reframe
    // panning happens on the .spill layer (its own touch-action:none), so drag-
    // to-reposition is unaffected. Reframe disables image touch via the rule below.
